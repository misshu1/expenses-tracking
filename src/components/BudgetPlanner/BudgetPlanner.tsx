...
const BudgetPlanner: React.FC = () => {
  ...
  const handleAddSubcategory = (catIdx: number) => {
    setCategories((categories) => {
      ...
      updated[catIdx].subcategories.push({ name: "", budget: 0, notes: "" });
      return updated;
    });
  };

  // Update category or subcategory
  const handleChange = (catIdx: number, field: keyof Category, value: any) => {
    setCategories((categories) => {
      const updated = [...categories];
  ...
  const handleRemoveSubcategory = (catIdx: number, subIdx: number) => {
    setCategories((categories) => {
      const updated = [...categories];
      updated[catIdx].subcategories = updated[catIdx].subcategories.filter(
        (_, i) => i !== subIdx,
      );
      return updated;
    });
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 max-w-5xl mx-auto flex flex-col gap-6">
      <h2 className="text-2xl font-bold mb-2 flex items-center gap-2 text-gray-900 dark:text-gray-100">
        <Icon icon="ri:calendar-event-line" width={28} /> Yearly Budget Planner
      </h2>
              ...
              >
                <td className="p-2 border border-gray-200 dark:border-gray-700">
                  <input
                    ...
                    value={cat.name}
                    onChange={(e) =>
                      handleChange(catIdx, "name", e.target.value)
                    }
                  />
                </td>
                <td className="p-2 border border-gray-200 dark:border-gray-700">
                  <input
                    ...
                    value={cat.budget}
                    onChange={(e) =>
                      handleChange(catIdx, "budget", Number(e.target.value))
                    }
                  />
                </td>
                <td className="p-2 border border-gray-200 dark:border-gray-700">
                ...
                <td className="p-2 border border-gray-200 dark:border-gray-700">
                  <input
                    ...
                    value={cat.notes}
                    onChange={(e) =>
                      handleChange(catIdx, "notes", e.target.value)
                    }
                  />
                </td>
                <td className="p-2 border border-gray-200 dark:border-gray-700">
                        ...
                        >
                          ...
                          <td>
                            <button
                              ...
                              onClick={() =>
                                handleRemoveSubcategory(catIdx, subIdx)
                              }
                            >
                              <Icon icon="ri:delete-bin-6-line" width={18} />
                            </button>
                          </td>
                        </tr>
                      ))}
